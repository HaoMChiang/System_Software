...
void fifo_schedule(void)
{
    ...
    if(readyQue.size() > 0 && occupied == 0) {
        pthread_cond_signal(&(cond[readyQue.at(0)]));
        readyQue.erase(readyQue.begin());
    }
    pthread_mutex_unlock(&mutex);
}

bool compareDeadline(int t1, int t2) {
    return tcb[t1].deadline < tcb[t2].deadline;
}
...
void edf_schedule(void)
{
    pthread_mutex_lock(&mutex);
    if(readyQue.size() > 0 && occupied == 0) {
        sort(readyQue.begin(), readyQue.end(), compareDeadline);
        pthread_cond_signal(&(cond[readyQue.at(0)]));
        readyQue.erase(readyQue.begin());
    }
    pthread_mutex_unlock(&mutex);
...
bool comparePeriod(int t1, int t2) {
    return tcb[t1].period < tcb[t2].period;
}
...
void rm_schedule(void)
{
    pthread_mutex_lock(&mutex);
    if(readyQue.size() > 0 && occupied == 0) {
        sort(readyQue.begin(), readyQue.end(), comparePeriod);
        pthread_cond_signal(&(cond[readyQue.at(0)]));
        readyQue.erase(readyQue.begin());
    }
    pthread_mutex_unlock(&mutex);
